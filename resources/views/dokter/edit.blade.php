    ...
    <div class="card mt-2">
      <div class="card-body">
        <form action="/dokter/update" method="post">
          ...
          <div class="row mb-2">
            <div class="col-md-4">
              <x-inputtext label="Kode Dokter" placeholder="Kode Dokter" value="{{$dokter->kode_dokter}}"
                field=" kode_dokter" readonly="true" icon="<svg xmlns='http://www.w3.org/2000/svg' class='icon' width='24' height='24' viewBox='0 0 24 24' stroke-width='2'
          ...
          <div class="row mb-2">
            <div class="col-md-12">
              <x-inputtext label="Nomor Induk Kependudukan (NIK)" placeholder="NIK" field="nik" value="{{$dokter->nik}}"
          ...
          <div class="rom mb-2">
            <x-inputtextarea label="Alamat" field="alamat" value="{{$dokter->alamat}}" />
          </div>
          <div class="row mb-2">
            <div class="col-md-6">
              ...
              <div class="form-group  @error('id_propinsi') is-invalid @enderror">
                <select name="id_propinsi" id="id_propinsi"
                  ...
                  <option @isset($dokter->id_propinsi) @if(old("id_propinsi"))
                    {{old("id_propinsi")==$p->id ? "selected":""}} @else
                    {{$dokter->id_propinsi==$p->id ? "selected":""}} @endif @else
          ...
          <div class="row mb-2">
            <div class="col-md-12">
              <x-inputtext label="No STR" placeholder="No STR" field="no_str" value="{{$dokter->no_str}}" icon="<svg xmlns='http://www.w3.org/2000/svg' class='icon' width='24' height='24' viewBox='0 0 24 24' stroke-width='2'
                stroke='currentColor' fill='none' stroke-linecap='round' stroke-linejoin='round'>
                <path stroke='none' d='M0 0h24v24H0z' fill='none' />
                <path d='M4 7v-1a2 2 0 0 1 2 -2h2' />
                <path d='M4 17v1a2 2 0 0 0 2 2h2' />
            ...
            <div class="col-md-6">
              ...
              <div class="form-group  @error('id_spesialis') is-invalid @enderror">
                <select name="id_spesialis" id="id_spesialis"
                  ...
                  <option @isset($dokter->id_spesialis) @if(old("id_spesialis"))
                    {{old("id_spesialis")==$p->id_spesialis ? "selected":""}} @else
                    {{$dokter->id_spesialis==$p->id_spesialis ? "selected":""}} @endif @else
                    {{old("id_spesialis")==$p->id_spesialis ? "selected":""}}
                    @endisset value="{{$p->id_spesialis}}">
                    {{$p->spesialis}}
                  </option>
                  @endforeach
                </select>
          ...
          <div class="row mb-2">
            <div class="col-md-6">
              <x-inputtext label="Tanggal Mulai Tugas" placeholder="Tanggal Mulai Tugas" field="tgl_mulai_tugas"
                value="{{$dokter->tgl_mulai_tugas}}"
                icon="<svg xmlns='http://www.w3.org/2000/svg' class='icon' width='24' height='24' viewBox='0 0 24 24' stroke-width='2' stroke='currentColor' fill='none' stroke-linecap='round' stroke-linejoin='round'><path stroke='none' d='M0 0h24v24H0z' fill='none'/><rect x='4' y='5' width='16' height='16' rx='2' /><line x1='16' y1='3' x2='16' y2='7' /><line x1='8' y1='3' x2='8' y2='7' /><line x1='4' y1='11' x2='20' y2='11' /><rect x='8' y='15' width='2' height='2' /></svg>" />
            </div>
          </div>
          <button class="btn btn-primary  w-100"><i class="fa fa-refresh mr-2"></i>Update</button>
        </form>
      </div>
    </div>
  </div>
...
<script>
  $(function() {   
    ...
    function loadkecamatan(){
      ...
      var id_kota ="";
      if(kota ==""){
        id_kota = "{{$dokter->id_kota}}";
      }else{
        id_kota = kota
      }
      var id_kecamatan = "{{$dokter->id_kecamatan}}";
      //alert(id_kota);
      $.ajax({
        type:'POST',
    ...
    function loadkelurahan(){
      ...
      var id_kecamatan ="";
      if(kecamatan ==""){
        id_kecamatan = "{{$dokter->id_kecamatan}}";
      }else{
        id_kecamatan = kecamatan;
      }

      var id_kelurahan = "{{$dokter->id_kelurahan}}";
      $.ajax({
        type:'POST',
